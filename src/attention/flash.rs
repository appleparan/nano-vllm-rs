...
mod tests {
    ...
    fn test_flash_attention_config() {
        let config = FlashAttentionConfig::new(64, true);
        assert_eq!(config.block_size_q, 64);
        assert_eq!(config.block_size_kv, 64);
        assert!(config.causal);
        assert!((config.softmax_scale - 0.125).abs() < 1e-6);
    }
    ...
    fn test_flash_attention_vs_naive() {
        // Compare Flash Attention output against naive SDPA
        let device = test_device();
        let batch = 1;
        let seq_len = 8;
        let num_heads = 2;
        ...
        assert!(
            max_diff < 1e-4,
            "Flash Attention output differs from naive SDPA by {max_diff}"
        );
    }
